...
def _largest_exterior(geometry):
    """Return the largest exterior ring from a Shapely geometry."""
    if geometry.is_empty:
        return np.empty((0, 2), dtype=np.float32)

    geometries = getattr(geometry, "geoms", (geometry,))
    polygons = [item for item in geometries if getattr(item, "geom_type", "") == "Polygon"]
    if not polygons:
        return np.empty((0, 2), dtype=np.float32)

    largest = max(polygons, key=lambda item: item.area)
    coordinates = np.asarray(largest.exterior.coords[:-1], dtype=np.float32)
    return coordinates.reshape(-1, 2)


def _shapely_offset(points, distance):
    """Offset a polygon without requiring the optional pyclipper package."""
    ...
    buffered = polygon.buffer(float(distance), join_style=1)
    return _largest_exterior(buffered)


def _offset_polygon(points, distance):
    """Return the largest offset polygon using pyclipper or a Shapely fallback."""
    points = np.asarray(points, dtype=np.float64).reshape(-1, 2)
    if len(points) < 3:
        return np.empty((0, 2), dtype=np.float32)

    if pyclipper is not None:
        subject = [tuple(map(int, np.rint(point))) for point in points]
        offset = pyclipper.PyclipperOffset()
        offset.AddPath(subject, pyclipper.JT_ROUND, pyclipper.ET_CLOSEDPOLYGON)
        paths = offset.Execute(float(distance))
        if paths:
            largest = max(paths, key=lambda item: abs(cv2.contourArea(np.asarray(item))))
            return np.asarray(largest, dtype=np.float32).reshape(-1, 2)

    return _shapely_offset(points, distance)
...
class SegDetectorRepresenter():
    ...
    def boxes_from_bitmap(self, pred, _bitmap, dest_width, dest_height):
        ...
        for index in range(num_contours):
            ...
            expanded = self.unclip(points, unclip_ratio=self.unclip_ratio)
            if expanded.size == 0:
                continue
    ...
    def unclip(self, box, unclip_ratio=1.5):
        ...
        expanded = _offset_polygon(box, distance)
        if expanded.size == 0:
            return np.empty((0, 0, 2), dtype=np.float32)
...
def shrink_polygon_pyclipper(polygon, shrink_ratio):
    ...
    if polygon_shape.is_empty or polygon_shape.length <= 0:
        return np.empty((0, 2), dtype=np.float32)
...
class MakeBorderMap():
    ...
    def draw_border_map(self, polygon, canvas, mask):
        ...
        padded_polygon = _offset_polygon(polygon, distance)
