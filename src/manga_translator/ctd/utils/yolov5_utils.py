...
def check_version(current='0.0.0', minimum='0.0.0', name='version ', pinned=False, hard=False):
    ...
    current, minimum = (Version(str(x)) for x in (current, minimum))
...
def _nms_pytorch(boxes: torch.Tensor, scores: torch.Tensor, iou_thres: float) -> torch.Tensor:
    """Pure PyTorch NMS to avoid hard dependency on torchvision.ops.nms."""
    if boxes.numel() == 0:
    ...
    while order.numel() > 0:
        ...
        rem = order[1:]
        xx1 = torch.maximum(x1[i], x1[rem])
        yy1 = torch.maximum(y1[i], y1[rem])
        xx2 = torch.minimum(x2[i], x2[rem])
        yy2 = torch.minimum(y2[i], y2[rem])
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        i = _nms_pytorch(boxes, scores, iou_thres)  # NMS
