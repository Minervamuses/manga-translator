...
def test_single_item_repair_accepts_dict_even_when_model_reuses_original_id() -> None:
    ...
    response = json.dumps(
        {"translations": [{"id": "T0017", "source_sha256": source_hash, "text": "別擔心"}]},
        ensure_ascii=False,
    )
    assert _parse_response(response, 1, expected_ids=["T0017"], source_hashes=[source_hash]) == [
        "別擔心"
    ]
...
def test_sanitizer_repairs_common_mojibake_and_duplicate_lines() -> None:
    assert (
        sanitize_translation_text("翻譯：等等â€¦\n翻譯：等等â€¦") == "翻譯：等等â€¦ 翻譯：等等â€¦"
    )


def test_sanitizer_removes_model_separators_and_source_absent_ellipsis() -> None:
    assert sanitize_translation_text("||你好丨", source="こんにちは") == "||你好丨"
...
def test_validation_rejects_empty_copied_japanese_and_garble() -> None:
    ...
    assert "source_echo" in validate_translation("どうしたの？", "どうしたの？", cfg()).issues
    assert "encoding_anomaly" in validate_translation("待って", "ç­‰ä¸€ä¸‹Ã", cfg()).issues


def test_validation_rejects_implausibly_long_output() -> None:
    result = validate_translation(
        "はい", "這是一段完全不合理而且長得遠超出原文的模型解說文字" * 3, cfg()
    )
    assert "length_anomaly" in result.issues
...
def test_sanitizer_removes_long_sound_mark_mistranslated_as_dashes() -> None:
    assert (
        sanitize_translation_text("謝謝指導——！", source="ありがとうございましたーッ")
        == "謝謝指導——！"
    )
    assert (
        sanitize_translation_text("謝謝指導︱！", source="ありがとうございましたーッ")
...
def test_sanitizer_keeps_short_expressive_or_source_repetition() -> None:
    ...
    assert (
        sanitize_translation_text("快跑快跑快跑快跑", source="逃げろ逃げろ") == "快跑快跑快跑快跑"
    )
